...
if [ ! -e "/opt/deepinwine/apps/Deepin-TIM/Initialized" ]; then
    if [ "$user"x != "root"x ]; then
    ...
    touch /opt/deepinwine/apps/Deepin-TIM/Initialized
fi
...
if [ "$1" ]; then
    ...
    echo "启动 TIM"
    nohup /opt/deepinwine/apps/Deepin-TIM/run.sh >"$logfile" &
    tail -fn 0 "$logfile"
fi
