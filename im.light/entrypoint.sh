...
if [ ! -e "/opt/deepinwine/apps/Deepin-QQLight/Initialized" ]; then
    if [ "$user"x != "root"x ]; then
    ...
    touch /opt/deepinwine/apps/Deepin-QQLight/Initialized
fi
...
if [ "$1" ]; then
    ...
    echo "启动 QQLight"
    nohup /opt/deepinwine/apps/Deepin-QQLight/run.sh >"$logfile" &
    tail -fn 0 "$logfile"
fi
